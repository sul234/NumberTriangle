...
public class NumberTriangle {
    ...
    public static NumberTriangle loadTriangle(String fname) throws IOException {
        ...
        while ((line = br.readLine()) != null) {
            String[] triangles = line.trim().split("\\s+");
            NumberTriangle[] rowNodes = new NumberTriangle[triangles.length];
            for (int i = 0; i < triangles.length; i++) {
                rowNodes[i] = new NumberTriangle(Integer.parseInt(triangles[i]));
            }
            rows.add(rowNodes);
        }
        br.close();
        ...
        for (int i = 0; i < rows.size() - 1; i++) {
            NumberTriangle[] curr = rows.get(i);
            NumberTriangle[] next = rows.get(i + 1);
            for ( int j = 0; j < curr.length; j++) {
                curr[j].setLeft(next[j]);
                curr[j].setRight(next[j + 1]);
            }
        }
